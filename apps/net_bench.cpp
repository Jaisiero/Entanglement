...
static void srv_on_data(const ent_packet_header *hdr, const uint8_t *payload, size_t payload_size, ent_endpoint sender,
                        void *ud)
{
    ...
    uint32_t mid = 0;
    int r = ent_server_send_to(ctx->server, payload, payload_size, hdr->channel_id, sender, 0, &mid);
    if (r >= 0)
        ctx->stats->echo_ok.fetch_add(1, std::memory_order_relaxed);
}

// Bulk coalesced callback: echo the entire coalesced packet in one raw send
// instead of unpacking N sub-messages and re-coalescing them.
static void srv_on_coalesced(const ent_packet_header *hdr, const uint8_t *raw_payload, size_t payload_size,
                             int message_count, ent_endpoint sender, void *ud)
{
    auto *ctx = static_cast<ServerCtx *>(ud);
    ctx->stats->recv.fetch_add(message_count, std::memory_order_relaxed);

    // Echo the raw coalesced payload back — prepare_header fills sequence/ack.
    ent_packet_header echo_hdr = *hdr;
    int r = ent_server_send_raw_to(ctx->server, &echo_hdr, raw_payload, sender);
    if (r >= 0)
        ctx->stats->echo_ok.fetch_add(message_count, std::memory_order_relaxed);
}
...
static uint8_t *srv_alloc(ent_endpoint, uint32_t, uint8_t, uint8_t, size_t max_size, void *ud)
{
    return static_cast<ServerCtx *>(ud)->allocs.alloc(max_size);
}

static void srv_complete(ent_endpoint sender, uint32_t, uint8_t ch, uint8_t *data, size_t total, void *ud)
{
    auto *ctx = static_cast<ServerCtx *>(ud);
    ctx->stats->recv.fetch_add(1, std::memory_order_relaxed);
...
static void srv_failed(ent_endpoint, uint32_t, uint8_t, uint8_t *buf, ent_message_fail_reason, uint8_t, uint8_t,
                       void *ud)
{
...
static void run_server(uint16_t port, int workers)
{
    ...
    if (workers > 0)
    {
        ent_server_set_use_async_io(srv, 1);
        ent_server_set_socket_count(srv, workers);
        std::printf("  Async I/O: ON  Sockets: %d (SO_REUSEPORT on Linux)\n", workers);
    }
    ...
    ent_server_set_on_coalesced_data(srv, srv_on_coalesced, &ctx);
    ...
    for (;;)
    {
        ...
        if (ms % 1000 < 2)
        {
            ...
            if (r != last_recv)
            {
                ...
                std::printf("  [%.0fs] recv:%llu (%llu/s)  echo:%llu\n", ms / 1000.0, (unsigned long long)r,
                            (unsigned long long)dr, (unsigned long long)e);
                last_recv = r;
            }
        }
...
static void cli_on_data(const ent_packet_header *, const uint8_t *, size_t, void *ud)
{
    static_cast<ClientCtx *>(ud)->stats->echoes.fetch_add(1, std::memory_order_relaxed);
}

static uint8_t *cli_alloc(ent_endpoint, uint32_t, uint8_t, uint8_t, size_t max_size, void *ud)
{
    return static_cast<ClientCtx *>(ud)->allocs.alloc(max_size);
}

static void cli_complete(ent_endpoint, uint32_t, uint8_t, uint8_t *data, size_t, void *ud)
{
...
static void cli_failed(ent_endpoint, uint32_t, uint8_t, uint8_t *buf, ent_message_fail_reason, uint8_t, uint8_t,
                       void *ud)
{
...
static void run_client(const char *server_ip, uint16_t port, int num_clients, int duration_secs, int payload_size,
                       int burst, bool coalesced)
{
    ...
    for (int i = 0; i < num_clients; ++i)
    {
        threads.emplace_back(
            [&stats, &running, i, server_ip, port, payload_size, burst, send_channel]()
            {
                ...
                while (running.load(std::memory_order_relaxed))
                {
                    ...
                    if (ent_client_can_send(cli))
                    {
                        for (int b = 0; b < burst; ++b)
                        {
                            if (!ent_client_can_send(cli))
                                break;
                            int r = ent_client_send(cli, buf.data(), buf.size(),
    ...
    for (int sec = 0; sec < duration_secs; ++sec)
    {
        ...
        std::printf("  [%.0fs] send:%llu/s  echo:%llu/s"
                    ...
                    t, (unsigned long long)(s - last_sent), (unsigned long long)(e - last_echo), (unsigned long long)s,
    ...
    double rate = total_sent > 0 ? 100.0 * (double)total_echo / (double)total_sent : 0.0;
...
int main(int argc, char *argv[])
{
    ...
    {
        std::fprintf(stderr, "Unknown mode: %s. Use 'server' or 'client'\n", argv[1]);
        return 1;
    }
