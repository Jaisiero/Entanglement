...
public:
    ...
    int send_to(const void* data, size_t size,
                const std::string& address, uint16_t port);

    // Receive raw data, fills sender address/port
    int recv_from(void* buffer, size_t buffer_size,
                  std::string& sender_address, uint16_t& sender_port);

    // Send a packet with header + payload
    int send_packet(const packet_header& header, const void* payload,
                    const std::string& address, uint16_t port);

    // Receive a packet, splits header and payload
    int recv_packet(packet_header& header, void* payload, size_t payload_capacity,
                    std::string& sender_address, uint16_t& sender_port);
