...
bool udp_socket::bind(uint16_t port, const std::string& address) {
    ...
    if (::bind(m_socket, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) != 0) {
        std::cerr << "[udp_socket] Failed to bind on port " << port
                  << ": " << last_socket_error() << std::endl;
        close();
        return false;
    }
...
int udp_socket::send_to(const void* data, size_t size,
...
int udp_socket::recv_from(void* buffer, size_t buffer_size,
...
int udp_socket::send_packet(const packet_header& header, const void* payload,
                             const std::string& address, uint16_t port) {
