...
namespace entanglement
{
    ...
    int udp_socket::send_gso_batch(const gso_batch_entry *entries, int count)
    {
        ...
        // Process in chunks of 256 to keep stack allocation bounded
        // while supporting larger batch pools (GSO_POOL_SLOTS > 128).
        static constexpr int CHUNK_SIZE = 256;
        int total_sent = 0;

        for (int chunk_start = 0; chunk_start < count; chunk_start += CHUNK_SIZE)
        {
            int chunk_count = std::min(CHUNK_SIZE, count - chunk_start);

            struct iovec iovs[CHUNK_SIZE];
            sockaddr_in addrs[CHUNK_SIZE];
            alignas(struct cmsghdr) char cmsg_bufs[CHUNK_SIZE][CMSG_SPACE(sizeof(uint16_t))];
            struct mmsghdr msgs[CHUNK_SIZE];

            std::memset(msgs, 0, sizeof(mmsghdr) * chunk_count);
            std::memset(cmsg_bufs, 0, sizeof(cmsg_bufs[0]) * chunk_count);

            for (int i = 0; i < chunk_count; i++)
            {
                const auto &e = entries[chunk_start + i];
